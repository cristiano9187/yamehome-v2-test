...
function App() {
  const [formData, setFormData] = useState<ReceiptData>({
    ...
    isCustomRate: false,
    customTotalAmount: 0,
    negotiatedNightlyRate: 0, // Initialized
    paidAmount: 0,
          ...
          <div className="bg-gray-800 p-4 rounded border border-gray-700">
            ...
            {formData.isCustomRate ? (
              <div className="mb-3 animate-fade-in">
                <label className="text-xs text-gray-400 block mb-1">Sous-total Séjour (Hors Caution)</label>
                <input 
                  type="number" 
                  name="customTotalAmount" 
                  className="w-full bg-gray-700 rounded p-2 border border-yellow-600 text-yellow-300" 
                  placeholder="Ex: 150000" 
                  onChange={handleChange} 
                />
                <p className="text-[10px] text-gray-500 mt-1 italic">
                  Entrez le montant total du loyer (Booking/Airbnb). La caution sera ajoutée automatiquement au total.
                </p>
              </div>
            ) : (
              <div className="mb-3 animate-fade-in">
                <label className="text-xs text-gray-400 block mb-1">Prix Négocié par nuit (Optionnel)</label>
                <input 
                  type="number" 
                  name="negotiatedNightlyRate" 
                  className="w-full bg-gray-700 rounded p-2 border border-gray-600 text-white" 
                  placeholder="Laisser vide pour prix standard" 
                  onChange={handleChange} 
                />
                <p className="text-[10px] text-gray-500 mt-1 italic">
                  Si rempli, ce prix remplace le tarif standard et calcule la réduction.
                </p>
              </div>
            )}
            ...
            <select name="paymentMethod" className="w-full bg-gray-700 rounded p-2 border border-gray-600" onChange={handleChange}>
