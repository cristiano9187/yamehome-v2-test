...
const ReceiptPreview: React.FC<ReceiptPreviewProps> = ({ data }) => {
  const calculateFinancials = () => {
    ...
    // Ensure nights is a valid number greater than 0
    if (isNaN(nights) || nights <= 0) return null;

    const standardRate = getRateForApartment(data.apartmentName, nights);
    
    // Logic split for Custom Rate vs Standard Rate
    let totalLodging = 0;
    let effectivePricePerNight = 0;

    if (data.isCustomRate) {
        // Custom Mode: User inputs TOTAL Subtotal directly
        totalLodging = data.customTotalAmount || 0;
        // We calculate the implied nightly rate for display purposes
        effectivePricePerNight = nights > 0 ? totalLodging / nights : 0;
    } else {
        // Standard Mode with potential Negotiation
        
        // Determine if a valid negotiated rate is entered
        const isNegotiated = data.negotiatedNightlyRate && data.negotiatedNightlyRate > 0;
        
        effectivePricePerNight = isNegotiated ? data.negotiatedNightlyRate : standardRate.prix;
        totalLodging = nights * effectivePricePerNight;
    }

    const caution = standardRate.caution; // Caution applies regardless of pricing mode
    const grandTotal = totalLodging + caution;
    const remaining = grandTotal - (data.paidAmount || 0);
    
    // Calculate discount
    let reductionPercent = 0;
    // Condition: Not Platform mode, and Effective Price is LESS than Standard Rule Price
  ...
  const startDateStr = data.startDate ? new Date(data.startDate).toLocaleDateString('fr-FR') : '';
  const endDateStr = data.endDate ? new Date(data.endDate).toLocaleDateString('fr-FR') : '';
  ...
  return (
    <div id="receipt-area" className="relative bg-white p-6 w-full max-w-[210mm] mx-auto text-gray-800 text-sm leading-relaxed shadow-lg print:shadow-none print:w-full print:p-4 print:text-xs">
      ...
      <div className="relative z-10 flex flex-col h-full justify-between min-h-[250mm] print:min-h-0 print:block">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div className="bg-gray-50 p-3 rounded-lg print:bg-transparent print:border print:border-gray-200">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs">Du {startDateStr} au {endDateStr}</p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs sm:text-sm">
                <tbody>
                  <tr className="border-b border-gray-100">
                    <td className="py-1">
                        Prix par nuit 
                        {data.isCustomRate && <span className="text-xs text-orange-600 ml-1">(Calculé sur Total)</span>}
                        {fin.reductionPercent > 0 && <span className="text-xs text-gray-400 line-through ml-2">{formatCurrency(fin.standardPricePerNight)}</span>}
                    </td>
                    <td className="py-1 text-right font-medium">{formatCurrency(fin.effectivePricePerNight)}</td>
                  </tr>
                  
                  {fin.reductionPercent > 0 && (
                    <tr className="border-b border-gray-100 text-green-700 bg-green-50 print:bg-transparent">
                        <td className="py-1 pl-2 text-xs italic">Réduction Appliquée</td>
                        <td className="py-1 text-right font-bold text-xs">-{fin.reductionPercent.toFixed(0)} %</td>
                    </tr>
                  )}
        ...
        {/* Footer Section: Signature and Logo */}
        <div className="mt-auto">
            ...
            {/* Footer Logo */}
            <div className="flex flex-col items-center justify-center border-t border-gray-200 pt-2 pb-2">
               <img src={LOGO_BASE64} alt="Logo Pied de page" className="h-10 opacity-90" />
               <p className="text-[9px] text-gray-400 mt-1 italic">Merci pour votre confiance !</p>
            </div>
        </div>
        
        <div className="text-center text-[8px] text-gray-300 print:hidden">
